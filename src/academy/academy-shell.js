...
(function(){
  ...
  window.openAcademy=function(){
    ...
    const cards=WORLDS.slice().sort((a,b)=>a.order-b.order).map(w=>{
      ...
      return `<button class="acWorld" onclick="openAcademyWorld('${w.id}')"><span class="acPlanet">${w.ic}</span><b>${esc(WORLD_NAMES[w.id]||w.id)}</b><small>${n} уроков · ${p}%</small><span class="acProg"><i style="width:${p}%"></i></span></button>`;
    }).join('');
    shell(`<div class="acHero"><div class="acEyebrow">MARKET AI · LEARNING SYSTEM</div><h2>От свечей до самостоятельного анализа</h2><p>Изучай рынок по шагам. Здесь прогресс оценивает обучение, а не обещает финансовый результат.</p><div class="acStats"><div class="acStat"><b>${EDUP.xp||0}</b><s>XP</s></div><div class="acStat"><b>${totalDone()}/${LESSONS.length}</b><s>Уроки</s></div><div class="acStat"><b>${availableCount()}</b><s>Доступно</s></div></div></div><div class="acSection"><b>Вселенная Академии</b><i></i></div><div id="acUniverse"></div>${window.MarketAILevels?MarketAILevels.render():''}${window.MarketAIAcademyProgress?MarketAIAcademyProgress.render():''}${window.MarketAIAdaptiveCoach?MarketAIAdaptiveCoach.render():''}<div class="acSection"><b>Миры списком</b><i></i></div><div class="acWorlds">${cards}</div><div class="acActions"><button class="acGhost" onclick="openAcademyContinue()">▶ Продолжить</button><button class="acGhost" onclick="openAcademyScenarioHub()">◈ Practice Lab</button><button class="acGhost" onclick="openAcademyReplayLibrary()">▶ Replay</button><button class="acGhost" onclick="openAcademyExam()">◆ Экзамен</button><button class="acGhost" onclick="openAcademySettings()">⚙ Настройки</button><button class="acGhost" onclick="openAcademyTour(true)">✦ Тур</button><button class="acGhost" onclick="openLegacyEducation()">📚 Старая теория</button></div>`,'MARKET AI ACADEMY','твоя карта обучения');
  ...
  window.openAcademyWorld=function(id){
    ...
    const rows=lessons.map(l=>{
      ...
      return `<button class="acLesson" ${av?'':'disabled'} onclick="openAcademyLesson('${l.id}')"><span class="acLessonIc">${esc(l.icon||'🎓')}</span><span class="acLessonM"><b>${esc(art?.t||l.id)}</b><span>${esc(art?.short||'Урок MARKET AI')} · ${l.estimatedTime||4} мин · +${l.xp||0} XP</span></span><span class="acState">${av?MASTERY_LABEL[st.mastery]:'🔒'}</span></button>`;
    }).join('')||'<div class="empty">Уроки этого мира ещё готовятся.</div>';
    shell(`<button class="acBack" onclick="openAcademy()">‹ Вселенная обучения</button><div class="acHero"><div class="acEyebrow">WORLD</div><h2>${w.ic} ${esc(WORLD_NAMES[w.id]||w.id)}</h2><p>${worldProgress(id)}% текущего учебного пути завершено.</p></div><div class="acSection"><b>Уроки</b><i></i></div><div class="acList">${rows}</div>`,'MARKET AI ACADEMY',WORLD_NAMES[w.id]||w.id);
  };
