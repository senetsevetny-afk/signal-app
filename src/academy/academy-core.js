...
const STEP_TYPES=['intro','character_message','explanation','chart','highlight_candle',
  ...
  'scenario_tree','comparison','drag_level','mark_structure','mark_structure_sequence','character_demo','practice_scene'];

/* ── Состояния персонажа, допустимые в уроках ── */
const CHAR_STATES=[
  /* состояния приложения (mascot) */
  'idle','loading','CALL','PUT','wait','analytics','history','think',
  'shield','globe','bell','listen','hero','wave','error',
  /* состояния сцены академии (character-scene.js) — единый словарь */
  'welcome','explain','explaining','point','pointing','thinking','question','warning','correct','incorrect','celebrate'
];
...
function normalizeCurriculumLesson(raw){
  if(!raw||!raw.id)return null;
  const mod=MODULES.find(m=>m.id===raw.moduleId);
  return {
    id:raw.id, worldId:raw.worldId||(mod?mod.worldId:null), moduleId:raw.moduleId||null,
    order:Number.isFinite(raw.order)?raw.order:999, icon:raw.icon||'🎓',
    title:raw.title||raw.name||raw.titleKey||raw.id, subtitle:raw.subtitle||raw.description||'',
    titleKey:raw.titleKey||null, subtitleKey:raw.subtitleKey||null, descriptionKey:raw.descriptionKey||null,
    difficulty:raw.difficulty||'beginner', estimatedTime:raw.estimatedTime||5, xp:raw.xp||20,
    prerequisites:Array.isArray(raw.prerequisites)?raw.prerequisites:[], characterState:raw.characterState||'idle',
    marketScene:raw.marketScene||null, steps:Array.isArray(raw.steps)?raw.steps:[],
    examples:Array.isArray(raw.examples)?raw.examples:[], counterExamples:Array.isArray(raw.counterExamples)?raw.counterExamples:[],
    quiz:Array.isArray(raw.quiz)?raw.quiz:[], practice:Array.isArray(raw.practice)?raw.practice:[], source:raw.source||'curriculum'
  };
}
function buildLessonRegistry(){
  const out=[];
  /* Keep legacy lessons as compatibility/fallback while curriculum grows. */
  (typeof EDU!=='undefined'?EDU:[]).forEach(a=>{
    const m=LESSON_MAP[a.id];
    if(!m){ window.logErr?.('LessonRegistry','нет привязки модуля для '+a.id); return }
    out.push(lessonFromArticle(a,m[0],m[1],m[2]));
  });
  const cur=window.MarketAIAcademy?.Curriculum;
  const extra=cur&&typeof cur.getLessons==='function'?cur.getLessons():[];
  const byId=new Map(out.map(l=>[l.id,l]));
  extra.map(normalizeCurriculumLesson).filter(Boolean).forEach(l=>byId.set(l.id,l));
  return Array.from(byId.values()).sort((a,b)=>{
    const wa=WORLDS.find(w=>w.id===a.worldId)?.order||999, wb=WORLDS.find(w=>w.id===b.worldId)?.order||999;
    if(wa!==wb)return wa-wb;
    const ma=MODULES.find(m=>m.id===a.moduleId)?.order||999, mb=MODULES.find(m=>m.id===b.moduleId)?.order||999;
    return ma!==mb?ma-mb:(a.order||0)-(b.order||0);
  });
}
let LESSONS=[];
...
function validateAcademy(){
  ...
  SCENARIOS.forEach(s=>{
    ...
    const ANSWERS=['up','down','skip','wait','enter','true','false',null];
    if(!ANSWERS.includes(s.correctAnswer))
      errors.push(`сценарий ${s.id}: недопустимый ответ ${s.correctAnswer}`);
    // null допустим: у структурных сценариев единственного верного ответа нет,
    // оценивается качество рассуждения, а не угадывание стороны
    const a=s.ohlc();
    if(Number.isFinite(s.candles)&&a.length!==s.candles)
      errors.push(`сценарий ${s.id}: свечей ${a.length}, ожидалось ${s.candles}`);
    if(!a.length)errors.push(`сценарий ${s.id}: пустой ряд свечей`);
    const b=s.ohlc();
